  ...
  <div id="tags-view-container" class="tags-view-container">
    <scroll-pane ref="scrollPaneRef" class="tags-view-wrapper" @scroll="handleScroll">
      <router-link v-for="tag in visitedViews" :key="tag.path" :data-path="tag.path"
        ...
        {{ tag.title }}
        <span v-if="!isAffix(tag)" @click.prevent.stop="closeSelectedTag(tag)" class="close-tag">
          <close class="el-icon-close el-icon" />
        </span>
      </router-link>
    </scroll-pane>
    <ul v-show="visible" :style="{ left: left+250 + 'px', top: top + 'px' }" class="contextmenu">
